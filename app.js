...
if (typeof window !== "undefined") {
  ...
  const DataAPI = {
    ...
    async fetchFlights({ direction = "all", sinceHours = 24, aheadHours = 24, limit = 800, fromISO = null, toISO = null } = {}) {
      let from, to;
      if (fromISO && toISO) {
        from = fromISO;
        to = toISO;
        ...
        from = israelWallClockISO(new Date(nowReal.getTime() - sinceHours * 3600 * 1e3));
        to = israelWallClockISO(new Date(nowReal.getTime() + aheadHours * 3600 * 1e3));
      }
      // Supabase/PostgREST caps every single request at ~1000 rows regardless
      // of .limit() — so a wide window (e.g. 14 days ≈ several thousand rows)
      // gets silently truncated to the newest 1000 rows (since we order
      // descending), quietly dropping everything older. Paginate with
      // .range() until we've collected up to `limit` rows or run out of data.
      const PAGE_SIZE = 1e3;
      const all = [];
      let offset = 0;
      while (all.length < limit) {
        let q = sb.from("flights").select("*").gte("scheduled_time", from).lte("scheduled_time", to).order("scheduled_time", { ascending: false }).range(offset, offset + PAGE_SIZE - 1);
  ...
  function OpenQuestionAI({ theme, subject, data, placeholder, hint }) {
    ...
    return /* @__PURE__ */ React.createElement("div", { className: `card rounded-2xl p-5 border shadow-sm space-y-3 ${!answer ? "no-print" : ""}` }, /* @__PURE__ */ React.createElement("h4", { className: "font-semibold text-primary text-sm flex items-center gap-2" }, "💬 שאלה חופשית ל-AI — ", subject), /* @__PURE__ */ React.createElement("p", { className: "text-xs text-muted no-print" }, hint || "שאלו כל שאלה על בסיס הנתונים שכבר מוצגים למעלה, ותקבלו תשובה מנוסחת מ-Gemini."), /* @__PURE__ */ React.createElement("div", { className: "flex gap-2 no-print" }, /* @__PURE__ */ React.createElement(
      "input",
      {
        value: question,
      ...
      loading ? "🔄 חושב..." : "▶️ שאל"
    )), error && /* @__PURE__ */ React.createElement("p", { className: "text-red-500 text-sm no-print" }, "⚠️ שגיאה: ", error), answer && /* @__PURE__ */ React.createElement("p", { className: "text-sm text-primary leading-relaxed whitespace-pre-line border-r-4 pr-4", style: { borderColor: theme.solid, animation: "fadeIn 0.3s ease-in" } }, answer));
  }
...
${csv}`;
  ...
  function WeeklyCountrySummary({ theme, countries, allMetrics }) {
    ...
    const load = useCallback(async () => {
      ...
      try {
        ...
        if (customRange) {
          ...
          const prevStart = new Date(startD.getTime() - durationMs - 1e3);
          rows = await DataAPI.fetchFlights({
            direction,
            fromISO: israelWallClockISO(prevStart),
            toISO: israelWallClockISO(endD),
            limit: 8e3
          });
        } else {
          rows = await DataAPI.fetchFlights({ direction, sinceHours: 24 * 14, aheadHours: 0, limit: 6e3 });
        }
        setFlights(rows);
      } catch (err) {
        console.error(err);
    ...
    const periodLabel = customRange ? `${customRange.start} – ${customRange.end}` : "7 הימים האחרונים";
    const { currentWeekRows, prevWeekRows } = useMemo(() => {
      if (customRange) {
        const startD = new Date(customRange.start + "T00:00:00");
        const endD = new Date(customRange.end + "T23:59:59");
        const durationMs = endD.getTime() - startD.getTime();
        const prevEnd = new Date(startD.getTime() - 1e3);
        const prevStart = new Date(startD.getTime() - durationMs - 1e3);
        const cur = [], prev = [];
        flights.forEach((f) => {
          const d = new Date(f.scheduled_time);
          if (isNaN(d.getTime())) return;
          if (d >= startD && d <= endD) cur.push(f);
          else if (d >= prevStart && d <= prevEnd) prev.push(f);
        });
    ...
    const kpis = useMemo(() => {
      ...
      return { totalCurrent, trendPct, activeCountries, activeCities, emerging };
    ...
    return /* @__PURE__ */ React.createElement("div", { className: "space-y-5" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center justify-between flex-wrap gap-3" }, /* @__PURE__ */ React.createElement("span", { className: "text-xs text-secondary flex items-center gap-1.5" }, "🛬 הנתונים כאן כוללים נחיתות בלבד — מיועד לניתוח מקורות תיירות נכנסת"), /* @__PURE__ */ React.createElement(ExportBar, null)), /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap items-center gap-3" }, /* @__PURE__ */ React.createElement("div", { className: "flex gap-1 p-1 rounded-2xl border", style: { background: hexA(theme.solid, 0.06), borderColor: hexA(theme.solid, 0.25) } }, [{ key: "country", label: "🗺️ לפי מדינה" }, { key: "city", label: "🏙️ לפי עיר" }].map((l) => /* @__PURE__ */ React.createElement(
    ...
    ), /* @__PURE__ */ React.createElement("div", { className: "flex flex-wrap items-center gap-1.5" }, /* @__PURE__ */ React.createElement("span", { className: "text-xs text-secondary" }, "📅 טווח מותאם:"), /* @__PURE__ */ React.createElement("input", { type: "date", value: pendingStart, onChange: (e) => setPendingStart(e.target.value), className: "input-field border rounded-lg px-2 py-1.5 text-xs" }), /* @__PURE__ */ React.createElement("span", { className: "text-xs text-muted" }, "עד"), /* @__PURE__ */ React.createElement("input", { type: "date", value: pendingEnd, onChange: (e) => setPendingEnd(e.target.value), className: "input-field border rounded-lg px-2 py-1.5 text-xs" }), /* @__PURE__ */ React.createElement("button", { onClick: applyCustomRange, disabled: !pendingStart || !pendingEnd, className: `text-xs px-3 py-1.5 rounded-lg text-white bg-gradient-to-l ${theme.grad} disabled:opacity-40` }, "החל טווח"), customRange && /* @__PURE__ */ React.createElement("button", { onClick: resetToDefault, className: "text-xs px-3 py-1.5 rounded-lg border text-secondary hoverable", style: { borderColor: "var(--card-border)" } }, "↩️ חזרה לשבוע אחרון")), level === "country" && /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-1.5" }, /* @__PURE__ */ React.createElement(
      "button",
      {
      ...
      if (isExpanded) {
        rowsOut.push(/* @__PURE__ */ React.createElement("tr", { key: `${s.key}-detail` }, /* @__PURE__ */ React.createElement("td", { colSpan: 8, className: "px-3 py-3 border-t divider", style: { background: "var(--hover-bg)" } }, /* @__PURE__ */ React.createElement("p", { className: "text-xs font-semibold text-secondary mb-2" }, "פילוח חברות תעופה עבור ", s.name, ":"), s.airlineList.length ? /* @__PURE__ */ React.createElement("div", { style: { maxWidth: 520 } }, /* @__PURE__ */ React.createElement(ChartCanvas, {
          type: "bar",
          data: {
            labels: s.airlineList.slice(0, 8).map(([name]) => name),
            datasets: [{ label: "טיסות", data: s.airlineList.slice(0, 8).map(([, count]) => count), backgroundColor: theme.solid, borderRadius: 4 }]
          },
          options: {
            indexAxis: "y",
            responsive: true,
            maintainAspectRatio: false,
            plugins: { legend: { display: false }, tooltip: { callbacks: { label: (ctx) => `${fmtNum(ctx.parsed.x)} טיסות (${Math.round(ctx.parsed.x / s.count * 100)}%)` } } },
            scales: { x: { ticks: { precision: 0 } } }
          },
          height: 180
        })) : /* @__PURE__ */ React.createElement("p", { className: "text-xs text-muted" }, "אין נתוני טיסות עבור ", s.name, " בשבוע האחרון."))));
      }
      return rowsOut;
  ...
  function FlightsMainTab({ theme, countries, allMetrics }) {
    return /* @__PURE__ */ React.createElement(WeeklyCountrySummary, { theme, countries, allMetrics });
  }
