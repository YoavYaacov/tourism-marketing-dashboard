(() => {
  ...
  function runRegression(allMetrics, years) {
    ...
    return { n, r2: Math.round(r2 * 100) / 100, influence, intercept: beta[0] };
  }
  ...
  function OpenQuestionAI({ country, metrics, theme }) {
    ...
    const ask = async () => {
      ...
      try {
        const res = await DataAPI.generateInsight("custom_question", {
          ...
          question: q,
          metrics: {
            sumVisitors: metrics.sumVisitors,
            sumOutbound: metrics.sumOutbound,
            sentiment: metrics.sentiment,
            religiousAffinity: metrics.religiousAffinity,
            roi: metrics.roi,
            growthTrend: metrics.growthTrend,
            totalScore: metrics.totalScore,
            avgHdi: metrics.avgHdi,
            avgGdpPerCapita: metrics.avgGdpPerCapita
          }
        });
